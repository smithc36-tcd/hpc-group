...
In this application, the message size is 4 Bytes. The number of receiving and sending messages is (the number of processes-1).  So the execution time is nearly $ T = (N-1)\times 2\times (s+4r) +9\times NUM_ITER/N\times t $, where N is the number of processes and t is the execution time per instrucation.
